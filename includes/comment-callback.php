<?php
function fireball_comment_callback($comment, $args, $depth)
{
    $tag=($args['style']==='div') ? 'div' : 'li';
    ?>
<<?php echo $tag; ?> id="<?php comment_ID(); ?>"
    <?php comment_class(['comment', $comment->comment_parent ? 'comment-child' : '']) ?>>
    <article class="comment-body" id="div-comment-<?php comment_ID(); ?>">
        ...
        <?php edit_comment_link(esc_html__('Edit Comment', 'fireball'), '<span class="comment-edit-link">', '</span>'); ?>
        <div class="comment-content">
            ...
            <a href="<?php echo esc_url(get_comment_link($comment, $args)) ?>" class="comment-time">
                <time datetime="<?php comment_time('c'); ?>"> <?php
                    printf(esc_html__('%s ago', 'fireball'), human_time_diff(get_comment_time('U'), current_time('U')));
                    ?> </time>
            </a>
            <?php if ($comment->comment_approved=='0') {?>
            <p class="comment-awaiting"><?php esc_html_e('Your comment is awaiting moderation', 'fireball') ;?></p>
            <?php } ?>
            <?php
