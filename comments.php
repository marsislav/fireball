...
$discussion = fireball_get_discussion_data();
?>
...
<div id="comments" class="<?php echo comments_open() ? 'comments-area' : 'comments-area comments-closed'; ?>">
    <div class="<?php echo $discussion->responses > 0 ? 'comments-title-wrap' : 'comments-title-wrap no-responses'; ?>">
		...
		if ( comments_open() ) {
			if ( have_comments() ) {
				_e( 'Join the Conversation', 'fireball' );
			} else {
				_e( 'Leave a comment', 'fireball' );
			}
		} else {
			if ( '1' == $discussion->responses ) {
				/* translators: %s: Post title. */
				printf( _x( 'One reply on &ldquo;%s&rdquo;', 'comments title', 'fireball' ), get_the_title() );
			} else {
				printf(
    ...
    <ol class="comment-list">
        <?php
			wp_list_comments(
				array(
					'walker'      => new fireball_Walker_Comment(),
					'avatar_size' => fireball_get_avatar_size(),
					'short_ping'  => true,
					'style'       => 'ol',
				)
			);
    ...
    <?php
		...
		if ( have_comments() ){
			$prev_icon     = fireball_get_icon_svg( 'chevron_left', 22 );
			$next_icon     = fireball_get_icon_svg( 'chevron_right', 22 );
			$comments_text = __( 'Comments', 'fireball' );
			the_comments_navigation(
				array(
					'prev_text' => sprintf( '%s <span class="nav-prev-text"><span class="primary-text">%s</span> <span class="secondary-text">%s</span></span>', $prev_icon, __( 'Previous', 'fireball' ), __( 'Comments', 'fireball' ) ),
					'next_text' => sprintf( '<span class="nav-next-text"><span class="primary-text">%s</span> <span class="secondary-text">%s</span></span> %s', __( 'Next', 'fireball' ), __( 'Comments', 'fireball' ), $next_icon ),
				)
			);
		}
    ...
    <div class="comment-form-flex comment-form-wrapper">
        <h2 class="comments-title"><?php _e( 'Leave a comment', 'fireball' ); ?></h2>
        <?php fireball_comment_form( 'asc' ); ?>
    </div>
    <?php
		endif;
		...
		if ( ! comments_open() ) :
			?>
    <p class="no-comments">
        <?php _e( 'Comments are closed.', 'fireball' ); ?>
    </p>
    <?php
		endif;
...
function fireball_get_discussion_data() {
	...
	function fireball_comment_form( $order ) {
	...
	class fireball_Walker_Comment extends Walker_Comment {
...
<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>"
        ...
        <footer class="comment-meta">
            <div class="comment-author vcard">
                <?php
							 ...
							 */
							if ( fireball_is_comment_by_post_author( $comment ) ) {
								printf( '<span class="post-author-badge" aria-hidden="true">%s</span>', fireball_get_icon_svg( 'check', 24 ) );
							}
	
							printf(
								wp_kses(
									/* translators: %s: Comment author link. */
									__( '%s <span class="screen-reader-text says">says:</span>', 'fireball' ),
									array(
										'span' => array(
											'class' => array(),
										),
            ...
            <div class="comment-metadata">
                <?php
							...
							$comment_timestamp = sprintf( __( '%1$s at %2$s', 'fireball' ), get_comment_date( '', $comment ), get_comment_time() );
							...
							$edit_comment_icon = fireball_get_icon_svg( 'edit', 16 );
							edit_comment_link( __( 'Edit', 'fireball' ), ' <span class="edit-link-sep">&mdash;</span> <span class="edit-link">' . $edit_comment_icon, '</span>' );
							?>
            ...
            <?php
						...
						if ( $commenter['comment_author_email'] ) {
							$moderation_note = __( 'Your comment is awaiting moderation.', 'fireball' );
						} else {
							$moderation_note = __( 'Your comment is awaiting moderation. This is a preview; your comment will be visible after it has been approved.', 'fireball' );
						}
						?>
	...
	function fireball_get_avatar_size() {
		return 100;
	}
	

	function fireball_is_comment_by_post_author( $comment = null ) {
	...
	function fireball_get_icon_svg( $icon, $size = 24 ) {
		return fireball_SVG_Icons::get_svg( 'ui', $icon, $size );
	}


	class fireball_SVG_Icons {
